// SiemensS7_x64.cpp : définit les routines d'initialisation pour la DLL.
//
#include "stdafx.h"
...
float DECL2 daveGetSeconds(daveConnection * dc) {
    ...
    f=(float)(b[0] & 0xf);//GCA101014
    f+=10*((b[0] & 0xf0)>>4);
    f+=100*(b[1] & 0xf);
    a=((b[1] & 0xf0)>>4);
    switch (a) {
	case 0: f*= (float)0.01;break;//GCA101014
	case 1: f*=(float)0.1;break;//GCA101014
	case 3: f*=(float)10.0;break;//GCA101014
    }
    return (f);    
}
/*	
...
int DECL2 _daveTimedRecv(daveInterface * di, uc *b, int len){
	...
	return recv((SOCKET)(di->fd.rfd),(char *) b, len, 0);
    }
#endif
...
//	    _daveDump("IBHfollow", IBHfollow, 15);
	    ...
	    res2=send((unsigned int)(di->fd.wfd),(const char *) IBHfollow, 15, 0);
...
#endif
#ifdef BCCWIN
    res = send((SOCKET)(dc->iface->fd.wfd),(const char *) dc->msgOut+dc->partPos, size, 0);
    if (res==SOCKET_ERROR )
...
void DECL2 _daveSendIt(daveInterface * di, uc * b, int size) {
    ...
    sum=sum & 0xff;
    b[size]=(unsigned char)sum;//GCA101014
    size++;
    b[size]=SYN;
    size++;
...
//    res=send((SOCKET)(di->fd.wfd), buffer, ((len+1)/2)*2, 0);
    res=send((SOCKET)(di->fd.wfd), (const char *)buffer, len, 0);
#endif
    return res;
}    
...
*/
    ...
    ts[3]=daveToBCD((unsigned char)t1.wMonth);//GCA101014
    ts[4]=daveToBCD((unsigned char)t1.wDay);//GCA101014
    ts[5]=daveToBCD((unsigned char)t1.wHour);//GCA101014
    ts[6]=daveToBCD((unsigned char)t1.wMinute);//GCA101014
    ts[7]=daveToBCD((unsigned char)t1.wSecond);//GCA101014
...
//    res=dc->iface->ifread(dc->iface, b1, /*dc->iface->timeout,*/ 2000 /*sizeof(__daveT1006)/2*/);
	...
	{//write bytes, compress & delblk
		...
		//if (!_daveReadSingle(dc->iface)==STX) 
		if (_daveReadSingle(dc->iface)!=STX) //GCA101014
		 {
...
//    fdr->id3= 0x1;
    co=0;
    do{
	a= _daveSCP_send((int)(dc->iface->fd.wfd), (unsigned char *) &reqBlock);
	er=SCP_get_errno();
	printf("res 7:%d %d\n",a,er);
#ifdef BCCWIN
	Sleep(100);
...
#endif    
#ifdef BCCWIN
    send((unsigned int)(di->fd.wfd),(const char *) c3, 3, 0);
#endif
...
#endif    
#ifdef BCCWIN
    send((unsigned int)(di->fd.wfd),(const char *) target, targetSize, 0);
#endif
